...
export default function (object) {
  ...
  const makeExtra = (function () {
    ...
    keys.forEach((item) => {
      let propDiv = divFactory('','extraProp', extraDiv)
      imgFactory(`${extraIcons[`${item}`]}`, propDiv, '', 'extrWicon');
      let textDiv = divFactory('','extraTextDiv',propDiv)
  ...
  for(let i = 0; i < 24;) {
    ...
    elementFactory('button',switchCont,`${i}`,` openH hidden`,'°')
    i = i + 8
  }
  ...
  hourly.forEach((item,index) => {
    ...
    let onehour = divFactory('', 'onehour', eigthHours);
